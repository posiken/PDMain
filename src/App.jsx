...
// ─── CONFIG ───────────────────────────────────────────────────────────────────
const STATUS_ORDER = { "trouble-call": 0, "in-training": 1, "pto": 2, "do-not-schedule": 3,
                        available: 0, "on-call": 2, "off-duty": 3 }; // legacy aliases
const TECH_TYPES   = ["GHP", "Lawn", "Termite", "Supervisor", "Trouble Call", "Production", "Support Staff"];
...
const TYPE_CFG     = {
  ...
  Production:      { color:"#fde047", bg:"rgba(253,224,71,.13)",  bd:"rgba(253,224,71,.32)"  },
  "Support Staff": { color:"#94a3b8", bg:"rgba(148,163,184,.13)", bd:"rgba(148,163,184,.32)" },
};
...
.type-btn.ghp-active  {border-color:#38bdf8;background:rgba(56,189,248,.18); color:#38bdf8;}
.type-btn.lawn-active {border-color:#4ade80;background:rgba(74,222,128,.18); color:#4ade80;}
.type-btn.tmte-active {border-color:#fb923c;background:rgba(251,146,60,.18);  color:#fb923c;}
.type-btn.spvr-active {border-color:#c084fc;background:rgba(192,132,252,.18);color:#c084fc;}
.type-btn.tc-active   {border-color:#f87171;background:rgba(248,113,113,.18);color:#f87171;}
.type-btn.prod-active {border-color:#fde047;background:rgba(253,224,71,.18); color:#fde047;}
.type-btn.ss-active   {border-color:#94a3b8;background:rgba(148,163,184,.18);color:#94a3b8;}
.type-btn.ghp-active:active  {border-color:#38bdf8;background:rgba(56,189,248,.18); color:#38bdf8;}
.type-btn.lawn-active:active {border-color:#4ade80;background:rgba(74,222,128,.18); color:#4ade80;}
.type-btn.tmte-active:active {border-color:#fb923c;background:rgba(251,146,60,.18);  color:#fb923c;}
.type-btn.spvr-active:active {border-color:#c084fc;background:rgba(192,132,252,.18);color:#c084fc;}
.type-btn.tc-active:active   {border-color:#f87171;background:rgba(248,113,113,.18);color:#f87171;}
.type-btn.prod-active:active {border-color:#fde047;background:rgba(253,224,71,.18); color:#fde047;}
.type-btn.ss-active:active   {border-color:#94a3b8;background:rgba(148,163,184,.18);color:#94a3b8;}
...
.tt-prod.on {border-color:rgba(253,224,71,.5); background:rgba(253,224,71,.1); color:#fde047;}
.tt-ss.on   {border-color:rgba(148,163,184,.5);background:rgba(148,163,184,.1);color:#94a3b8;}
...
const TYPE_ACTIVE_CLASS = {
  GHP:"ghp-active", Lawn:"lawn-active", Termite:"tmte-active", Supervisor:"spvr-active",
  "Trouble Call":"tc-active", Production:"prod-active", "Support Staff":"ss-active"
};
const TYPE_SUB = {
  GHP:"General Household", Lawn:"Lawn & Outdoor", Termite:"Termite Control",
  Supervisor:"Lead / Oversight", "Trouble Call":"Trouble Calls", Production:"Production Routes", "Support Staff":"Support / Admin"
};
      ...
      <div className="search-hero">
        ...
        <div className="type-section">
          ...
          <div className="type-grid" style={{gridTemplateColumns:"repeat(3,1fr)"}}>
            {["GHP","Lawn","Termite"].map(type=>(
              <button key={type}
                className={`type-btn${selTypes.includes(type)?` ${TYPE_ACTIVE_CLASS[type]}`:""}`}
          ...
          <div className="type-grid" style={{gridTemplateColumns:"repeat(2,1fr)"}}>
            {["Supervisor","Support Staff"].map(type=>(
              <button key={type}
                className={`type-btn${selTypes.includes(type)?` ${TYPE_ACTIVE_CLASS[type]}`:""}`}
                disabled={!zipReady} onClick={()=>toggleType(type)}>
                <div className="type-btn-label">{type}</div>
...
// ─── TECH MODAL ───────────────────────────────────────────────────────────────
const TT_CLASS = { GHP:"tt-ghp", Lawn:"tt-lawn", Termite:"tt-tmte", Supervisor:"tt-spvr", "Trouble Call":"tt-tc", Production:"tt-prod", "Support Staff":"tt-ss" };
function TechModal({ mode, tech, allTechs, onSave, onClose }) {
  const blank = {name:"",pestpacUsername:"",phone:"",status:"trouble-call",branch:"",types:[],zipCodes:[],notes:""};
    ...
    <div className="overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal">
        ...
        <div className="field">
          ...
          <input className="field-input" value={form.pestpacUsername} onChange={e=>upd("pestpacUsername",e.target.value)} placeholder="e.g. jsmith" style={{fontFamily:"'DM Mono',monospace",fontSize:13}}/>
        </div>
        <div className="field">
        ...
        <div style={{display:"flex",gap:12}}>
          <div className="field" style={{flex:1}}>
            ...
            <select className="field-input" value={form.status} onChange={e=>upd("status",e.target.value)}>
              <option value="trouble-call">Trouble Call</option>
