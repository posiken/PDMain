...
.type-btn.tc-active   {border-color:rgba(248,113,113,.55);background:rgba(248,113,113,.1);color:#f87171;}
.type-btn.prod-active {border-color:rgba(253,224,71,.55); background:rgba(253,224,71,.1); color:#fde047;}
.type-btn.ss-active   {border-color:rgba(148,163,184,.55);background:rgba(148,163,184,.1);color:#94a3b8;}
...
.type-toggle{padding:10px 6px;border-radius:7px;border:1.5px solid #1a2a3d;background:#090e1a;
  color:#3d5068;cursor:pointer;transition:all .18s;font-family:'Barlow Condensed',sans-serif;font-size:15px;font-weight:700;letter-spacing:.04em;text-align:center;}
...
.copy-btn{padding:2px 9px;background:transparent;border:1px solid #1a2a3d;border-radius:4px;
  color:#3d5068;font-family:'DM Mono',monospace;font-size:10px;letter-spacing:.04em;cursor:pointer;
...
.paste-toggle{background:none;border:none;color:#3d5068;font-family:'DM Mono',monospace;
...
.bulk-textarea::placeholder{color:#263047;}
...
.footer-copy{flex:2;min-width:260px;font-size:12px;color:#263047;line-height:1.75;}
.footer-divider{width:1px;background:#0f1826;align-self:stretch;flex-shrink:0;}
.footer-contact{flex:1;min-width:190px;}
.footer-contact-label{font-family:'DM Mono',monospace;font-size:9px;letter-spacing:.14em;text-transform:uppercase;color:#1e2d42;margin-bottom:11px;}
.footer-contact-name{font-family:'Barlow Condensed',sans-serif;font-size:18px;font-weight:700;color:#3d5068;margin-bottom:7px;}
.footer-link{display:block;font-family:'DM Mono',monospace;font-size:12px;color:#263047;text-decoration:none;margin-bottom:4px;transition:color .18s;letter-spacing:.04em;}
    ...
    <div className="tech-card" style={{animationDelay:`${index*55}ms`}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        <div style={{display:"flex",alignItems:"center",gap:9,marginBottom:5,flexWrap:"wrap"}}>
          ...
          {tech.branch && (
            <span style={{fontFamily:"'DM Mono',monospace",fontSize:10,color:"#3d5068",letterSpacing:".06em"}}>{tech.branch}</span>
          )}
        </div>
        <a href={`tel:${tech.phone}`} className="tech-phone">{tech.phone}</a>
        {tech.pestpacUsername && (
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:7}}>
            <span style={{fontFamily:"'DM Mono',monospace",fontSize:11,color:"#3d5068",letterSpacing:".04em"}}>
              PestPac:&nbsp;<span style={{color:"#64748b"}}>{tech.pestpacUsername}</span>
            </span>
            <button className={`copy-btn${copied?" copy-btn-ok":""}`} onClick={handleCopy}>
              {copied ? "✓ Copied!" : "Copy"}
            </button>
          </div>
        )}
...
function SearchView({ techs, zipInput, setZipInput, result, setResult }) {
  ...
  useEffect(()=>{
    ...
    const matches = techs
      .filter(t => t.zipCodes.includes(zip) && selTypes.every(st=>(t.types||[]).includes(st)))
          ...
          <div className="type-label" style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            ...
            {selTypes.length>0 && (
              <button onClick={()=>setSelTypes([])}
                style={{background:"none",border:"none",color:"#3d5068",fontFamily:"'DM Mono',monospace",
            ...
            <span className="results-label">
              ...
              {result.types.map((t,i)=>(
                <span key={t}>
                  ...
                  {i<result.types.length-1&&<span style={{color:"#263047"}}>&nbsp;+&nbsp;</span>}
                </span>
              ))}
            </span>
                ...
                <tbody>
                  {techs.map(tech=>(
                    <tr key={tech.id}>
                      ...
                      <td style={{color:"#3d5068",fontSize:13,maxWidth:140}}>{tech.notes||"—"}</td>
                      <td>
                        <div style={{display:"flex",gap:7}}>
