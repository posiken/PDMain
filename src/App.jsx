...
.type-btn-label{font-family:'Barlow Condensed',sans-serif;font-weight:900;letter-spacing:.08em;text-transform:uppercase;white-space:nowrap;overflow:hidden;}
...
.type-toggle{padding:10px 6px;border-radius:7px;border:1.5px solid #1a2a3d;background:#090e1a;
  color:#64748b;cursor:pointer;transition:all .18s;font-family:'Barlow Condensed',sans-serif;font-weight:700;letter-spacing:.08em;text-align:center;text-transform:uppercase;white-space:nowrap;overflow:hidden;}
...
// Returns a font size (px) that keeps the label on one line across all button widths
const typeFontSize = len =>
  len <= 5 ? 17 : len <= 7 ? 15 : len <= 9 ? 13 : len <= 12 ? 11 : 10;
          ...
          <div className="type-grid" style={{gridTemplateColumns:"repeat(4,1fr)"}}>
            {["GHP","Lawn","Termite","Mosquito"].map(type=>(
              <button key={type} className="type-btn"
                ...
                <div className="type-btn-label" style={{fontSize:typeFontSize(type.length)}}>{type}</div>
              </button>
            ))}
          </div>
      ...
      <div className="modal">
        ...
        <div className="field">
          ...
          <div className="type-toggle-row">
            {TECH_TYPES.map(type=>(
              <button key={type} className="type-toggle" style={{fontSize:typeFontSize(type.length),...(form.types.includes(type)?{borderColor:"rgba(245,158,11,.55)",background:"rgba(245,158,11,.13)",color:"#fbbf24"}:{})}}
